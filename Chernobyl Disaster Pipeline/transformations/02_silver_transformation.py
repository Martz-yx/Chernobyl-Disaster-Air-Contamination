...
def Chernoclean():
    ...
    pays_col = col("PAYS")
    for code, country in pays_country_map.items():
...
def weatherclean():
    ...
    df = (
        ...
        .withColumn("City", expr("split(NAME, ',')[0]"))
        .withColumn("Country", expr("trim(split(NAME, ',')[1])"))
        .withColumnRenamed("PRCP", "Precipitation")
        .drop("PRCP_ATTRIBUTES", "SNOW_ATTRIBUTES", "SNOW", "TAVG_ATTRIBUTES", "NAME","STATION", "ELEVATION")
    )
    country_col = col("Country")
    for code, country in fips_country_map.items():
        df = df.withColumn("Country", when(country_col == code, country).otherwise(col("Country")))
        country_col = col("Country")
        df = (
        df.withColumn(
            "City",
            when(col("City").isin(
                "BERLIN ZEHLENDORF", "BERLIN DAHLEM", "BERLIN TEGEL", "BERLIN TEGELER FLIESSTAL"
            ), "BERLIN-WEST")
            .when(col("City") == "BERLIN BUCH", "BERLIN-EAST")
            .when(col("City").isin(
                "BERLIN BUCH", "BERLIN TEGEL", "BERLIN TEGELER FLIESSTAL"
            ), "BERLIN-NORTH")
            .when(col("City").isin(
                "BERLIN LICHTENRADE", "BERLIN SCHONEFELD", "BERLIN TEMPELHOF"
            ), "BERLIN-SOUTH")
            .otherwise(col("City"))
        )
    )
